...
# join data
apt_info_df = apt_df.join(invoices_df.set_index('appointment_id'), on='Appointment ID').join(agent_df.set_index('appointment_id'), on='Appointment ID').sort_values('Appointment ID', ascending=False)
...
apt_info_df_with_items_pivoted = apt_info_df_with_items.join(item_df.set_index('item_apt_id'), on='Appointment ID')
...
# Type Report column
apt_info_df2['Type Report'] = domo.type_report(apt_info_df2)
...
# item grouping
group_df_filled = domo.load_gsheet('https://docs.google.com/spreadsheets/d/1SoATnnqdqB66XN0En7ivt8T6odGynGWSNYajqar8O6k/').iloc[:, [0,3]].dropna(subset='Appointment ID')
...
group_df = apt_info_df2.loc[cond1 & cond2, col].drop_duplicates(subset='item_name').sort_values('Appointment ID')

# upload to gsheet and fill in item groups manually
domo.update_gsheet('https://docs.google.com/spreadsheets/d/1SoATnnqdqB66XN0En7ivt8T6odGynGWSNYajqar8O6k/', group_df)

# download filled data from gsheet
group_df_filled = domo.load_gsheet('https://docs.google.com/spreadsheets/d/1SoATnnqdqB66XN0En7ivt8T6odGynGWSNYajqar8O6k/').iloc[:,[0,3]].dropna(subset='item_name_group')
"""
apt_info_df3 = apt_info_df2.join(group_df_filled.set_index('Appointment ID'), on='Appointment ID')


# travel fee
patient_address_df = domo.get_travel_fee(apt_info_df3.dropna(subset='Patient Address').drop_duplicates(subset='Appointment ID'))
...
apt_final_df['Phone Number'] = domo.clean_phone_number(apt_info_df3['Phone Number'].reset_index(drop=True))
...
apt_final_df['Doctor Name'] = np.where(apt_info_df3['graduate_id']!=13, apt_info_df3['Doctor Name'], '')
...
apt_final_df['Nurse Name'] = np.where(apt_info_df3['graduate_id']==13, apt_info_df3['Doctor Name'], '')
...
apt_final_df['Item SKU'] = np.where(apt_info_df3['item_id'].isna(), '', apt_info_df3['Clinic ID'].astype('Int64').astype(str) + '_' + apt_info_df3['item_id'].astype('Int64').astype(str))
apt_final_df['Item Unit Price'] = apt_info_df3['item_price'].astype(float).astype('Int64').fillna(0)
apt_final_df['Item Quantity'] = apt_info_df3['item_quantity'].astype(float).astype('Int64').fillna(0)
apt_final_df['Item Subtotal'] = apt_final_df['Item Unit Price'] * apt_final_df['Item Quantity']
...
#####################################################################
# LOAD
# domo.update_gsheet('https://docs.google.com/spreadsheets/d/1toxh7WoGWurp1F0R_IEhb_8KU82twtE7EClSRQMZmu4/', apt_final_df)
